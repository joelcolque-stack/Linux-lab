 ...
 PCT=0
 if [ " $TOTAL " - gt 0 ]; then
 PCT=$(awk "BEGIN {printf \ " %.1 f \ " , ($COUNT/$TOTAL)*100}")
 fi
 ...
 sort | \
 uniq -c | \
 awk ’{printf" %s:00 %s evento\ n", $2, $1}’
 echo ""
 ...
 cat > " $REPORT " << EOF
 # Reporte de A n l i s i s de Logs
...
| Hora | Eventos |
 |------|---------|
 $ ( cut -d ’| ’ - f1 " $LOGFILE " | awk ’{ print $2 } ’ | cut -d ’: ’ -
f1 | \
 sort | uniq -c | awk ’{ printf "| % s :00 | % d |\ n " , $2 ,
$1 } ’)
...
| \
 ...
 awk ’{ count = $1 ; $1 =""; gsub (/^ / ,"" , $0 ) ; printf "| % d |
% s |\ n " , count , $0 } ’)

 # # 5. Resumen

 - Sistema analizado con $ ( wc -l < " $LOGFILE " ) eventos registrados
 - $ ( grep -c " | ERROR \|| FATAL | " " $LOGFILE " 2 >/ dev / null ||echo 0) eventos requieren a t e n c i n ( ERROR y FATAL )
 - Analisis completado con herramientas UNIX estandar EOF
